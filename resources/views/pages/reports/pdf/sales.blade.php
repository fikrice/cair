...
<head>
    ...
    <title>Sales Report - PT Mesama Global Indonesia</title>
    <style>
    ...
    <table class="header-table">
        <tr>
            ...
            <td width="40%" class="report-title-cell">
                <h1 class="report-title-main">Laporan Penjualan</h1>
            </td>
        </tr>
    </table>
    ...
    <table class="summary-table">
        <tr>
            <td class="summary-cell">
                ...
                <span class="summary-value">{{ $summary['total_orders'] }} Orders</span>
            </td>
            <td class="summary-cell">
                <span class="summary-label">Total Penjualan (Gross)</span>
                <span class="summary-value">Rp {{ number_format($summary['total_revenue'], 0, ',', '.') }}</span>
            </td>
            <td class="summary-cell">
                <span class="summary-label">Total Pembayaran (Net)</span>
                <span class="summary-value">Rp {{ number_format($summary['total_paid'], 0, ',', '.') }}</span>
            </td>
        </tr>
    </table>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="font-bold">{{ $so->so_number }}</td>
                <td style="text-transform: uppercase;">{{ $so->customer_name }}</td>
                <td class="text-right font-bold">{{ number_format($so->total_amount, 0, ',', '.') }}</td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
            <tr style="background-color: #eee;">
                <td colspan="4" class="text-right font-bold" style="padding: 10px; text-transform: uppercase;">Total Akumulasi Penjualan</td>
                <td class="text-right font-bold" style="padding: 10px; font-size: 11px;">Rp {{ number_format($summary['total_revenue'], 0, ',', '.') }}</td>
            </tr>
        </tfoot>
    </table>
