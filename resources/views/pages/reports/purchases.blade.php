...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-black text-gray-900 dark:text-white tracking-tight">Procurement Intelligence</h1>
            <p class="text-sm font-medium text-gray-500">Analisis pengeluaran stok, performa supplier, dan efisiensi belanja.</p>
        </div>
        <nav>
            <ol class="flex items-center gap-2 text-sm">
                <li><a class="font-bold text-gray-400 hover:text-brand-500 transition" href="{{ route('dashboard') }}">ERP /</a></li>
                <li class="font-bold text-brand-500">Purchasing Analytics</li>
            </ol>
        </nav>
    </div>
    ...
    <div class="flex flex-col gap-6 mb-8 lg:flex-row">
        ...
        <div class="w-full lg:w-80 shrink-0">
            <div class="h-full rounded-3xl border border-gray-200 bg-white p-6 shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03]">
                <div class="flex items-center gap-3 mb-6">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-error-500 text-white shadow-lg shadow-error-500/20">
                ...
                <form action="{{ route('reports.purchases') }}" method="GET" class="space-y-6">
                    <div x-data="{
                        init() {
                    ...
                    <div class="flex flex-col gap-3 pt-4">
                        <button type="submit" class="w-full rounded-2xl bg-brand-500 py-4 text-xs font-black text-white hover:bg-brand-600 transition shadow-xl shadow-brand-500/25 active:scale-95 uppercase tracking-widest">
                            Update Procurement View
                        </button>
                        <a href="{{ route('reports.purchases') }}" class="w-full inline-flex items-center justify-center rounded-2xl border-2 border-gray-100 bg-white py-3.5 text-xs font-bold text-gray-400 hover:bg-gray-50 transition dark:border-gray-800 dark:bg-transparent uppercase tracking-widest">
                            Reset View
                        </a>
                    </div>
                </form>
        ...
        <div class="flex-1">
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 h-full">
                <!-- Total Orders -->
                <div class="relative overflow-hidden rounded-3xl border border-gray-200 bg-white p-8 shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="relative z-10">
                        ...
                        <h4 class="text-[11px] font-black text-gray-400 uppercase tracking-[2px] mb-2">Total PO Issued</h4>
                        <div class="flex items-baseline gap-2">
                            <span class="text-4xl font-black text-gray-900 dark:text-white">{{ $summary['total_orders'] }}</span>
                            <span class="text-xs font-bold text-gray-400">Transactions</span>
                        </div>
                        <p class="mt-4 text-xs font-medium text-gray-500 leading-relaxed">Jumlah Purchase Order yang telah dikirimkan ke supplier dalam periode ini.</p>
                    </div>
                    <div class="absolute -right-6 -bottom-6 text-brand-500/5 rotate-12">
                        <svg width="120" height="120" viewBox="0 0 24 24" fill="currentColor"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/></svg>
                    </div>
                </div>
                ...
                <div class="relative overflow-hidden rounded-3xl border border-gray-200 bg-white p-8 shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03]">
                    <div class="relative z-10">
                        <div class="flex h-14 w-14 items-center justify-center rounded-2xl bg-error-50 text-error-600 dark:bg-error-500/10 mb-6">
                        ...
                        <h4 class="text-[11px] font-black text-gray-400 uppercase tracking-[2px] mb-2">Total Pengeluaran</h4>
                        <div class="flex flex-col">
                            <span class="text-sm font-bold text-error-600 mb-1">Expenditure Value</span>
                            <span class="text-3xl font-black text-gray-900 dark:text-white leading-tight">Rp {{ number_format($summary['total_expense'], 0, ',', '.') }}</span>
                        </div>
                        <p class="mt-4 text-xs font-medium text-gray-500 leading-relaxed">Total nilai belanja stok untuk menjaga ketersediaan aset di gudang.</p>
                    </div>
                    <div class="absolute -right-6 -bottom-6 text-error-500/5 rotate-12">
    ...
    <div class="rounded-3xl border border-gray-200 bg-white shadow-theme-sm dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden">
        <div class="flex flex-col gap-4 px-8 py-8 sm:flex-row sm:items-center sm:justify-between bg-gray-50/50 dark:bg-white/[0.02]">
            <div>
                <h3 class="text-lg font-black text-gray-900 dark:text-white">Procurement Logs</h3>
                <p class="text-xs font-medium text-gray-500">Rincian detail riwayat pembelian stok ke berbagai supplier.</p>
            </div>
            ...
            <table class="w-full border-collapse">
                <thead>
                    <tr class="bg-white dark:bg-transparent">
                        ...
                        <th class="px-8 py-5 text-start text-xs font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800">PO Number</th>
                        <th class="px-8 py-5 text-start text-xs font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800">Supplier Vendor</th>
                        <th class="px-8 py-5 text-start text-xs font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800">Purchasing PIC</th>
                        <th class="px-8 py-5 text-end text-xs font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 dark:border-gray-800">Total Purchase</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors group">
                        ...
                        <td class="px-8 py-5 text-sm font-bold text-gray-900 dark:text-white">{{ $po->po_date->format('d M, Y') }}</td>
                        <td class="px-8 py-5">
                            <span class="inline-flex items-center px-3 py-1.5 rounded-lg bg-error-50 text-error-700 text-sm font-black dark:bg-error-500/10 dark:text-error-400 font-mono">
                                {{ $po->po_number }}
                            </span>
                        </td>
                        <td class="px-8 py-5">
                            <div class="flex flex-col">
                                <span class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">{{ $po->supplier->name }}</span>
                                <span class="text-xs font-bold text-gray-400">Verified Vendor</span>
                            </div>
                        </td>
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-3">
                                <div class="h-8 w-8 rounded-xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center text-xs font-black text-gray-600 border border-gray-200 dark:border-gray-700">
                                    {{ substr($po->creator->name, 0, 1) }}
                                </div>
                                <span class="text-xs font-bold text-gray-600 dark:text-gray-400">{{ $po->creator->name }}</span>
                            </div>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="px-8 py-24 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-base font-bold text-gray-400">No purchase records found.</p>
                ...
                <tfoot class="bg-gray-900 dark:bg-white/[0.05]">
                    <tr class="font-black">
                        <td colspan="5" class="px-8 py-6 text-end text-xs text-gray-400 uppercase tracking-[3px]">Total Consolidated Expenditure</td>
                        <td class="px-8 py-6 text-end text-xl text-white">Rp {{ number_format($summary['total_expense'], 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
                @endif
            </table>
